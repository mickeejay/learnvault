...
This guide demonstrates how to use the `/api/credentials/metadata` endpoint to
generate NFT metadata and mint ScholarNFT credentials.
...
const tx = contract.call("mint", learnerAddress, metadataUri)
...
- The `completed_at` timestamp must be in ISO 8601 format (e.g.,
...
1. Upload actual badge images to IPFS and update the `IMAGE_CID_MAP` in
